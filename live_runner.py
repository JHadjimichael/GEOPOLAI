...
tick_one = sim_bed.tick()
tick_one['timestamp'] = "0"
data_over_time = [tick_one]

countries = [country for entry in data_over_time[0] if entry != 'timestamp' for country in [entry]]

graph = plt.plot(tick_one["timestamp"], tick_one[])

def visualize_statistic_over_time(countries, statistic, num_ticks):
    for i in range(num_ticks-1):
        new_data = sim_bed.tick()
        new_data['timestamp'] = str(i+1)
        data_over_time.append(new_data)
...
visualize_statistic_over_time(countries, istatistic, 10)